...
options = {
    'E12': [round(base * (10 ** i),1) for i in range(7) for base in E12base],
    'E24': [round(base * (10 ** i),1) for i in range(7) for base in E24base],
    'E48': [round(base * (10 ** i),2) for i in range(7) for base in E48base]
}

def CalculateVout(R1, R2, R3, vin):
    if R2 == math.inf:
        R23 = R3
    elif R3 == math.inf:
        R23 = R2
...
def main():
    if len(sys.argv) != 4:
        print('\n\nUsage: python ./VoltageDivider.py [high voltage] [low voltage] [resistance series (E12/E24/E48)]\n\n')
        sys.exit(1)
    
    try:
        vin = float(sys.argv[1])
        vtarget = float(sys.argv[2])
        series = sys.argv[3]
    except ValueError:
    ...
    print(' ===============================================================')
    print(f'|   {vin:.2f}V ---------( R1 )--\t---+-----( R2 )-----+--- Gnd\t|')
    print('|                           \t   |                |      \t|')
    print('|                           \t   +-----( R3 )-----+      \t|')
    print('|                           \t   |                       \t|')
    print(f'|                           \t   +----- {vtarget:.2f}V           \t|')
    print(' ===============================================================')
    ...
    for i in range(len(options[series])):
        for j in range(i,len(options[series])):
            R2 = options[series][i]
            R3 = options[series][j]
            vout = CalculateVout(R1def, R2, R3, vin)
    ...
    for R2 in options[series]:
        vout = CalculateVout(R1def, R2, math.inf, vin)
        err = abs((vout-vtarget)/vtarget)
    ...
    for config in configs:
        print(f"R1={tools.Notation(R1def)}Ohms,\
              ...
              Vout={tools.Notation(config[2])}V,\
